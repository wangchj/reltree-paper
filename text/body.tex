...
\begin{lstlisting}[language=java, caption=Root for Adjacency List, label=lst_aj_root]
...
\noindent{\bf Parent, Children, Siblings}
...
\noindent{\bf Leaves, Height, Depth, Path, Member}
...
\begin{lstlisting}[language=java, caption=Leaves, label=lst_aj_leaves]
...
\begin{lstlisting}[language=java, caption=Member, label=lst_aj_member]
...
\begin{lstlisting}[language=java, caption=Tree via Adjacency List, label=lst_aj_tree]
...
\begin{lstlisting}[language=java, caption={Algorithm for Left-Right value assignment for nested set model},label=ns_assignlr]
...
\item On the up-trip (line 13), $R$ of $node$ is assigned $count + 1$. If node is leaf, then $R$ is assigned, $L + $. Else, $R$ is assigned $R$ of last child + 1.
